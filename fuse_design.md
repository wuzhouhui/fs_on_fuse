...
* 数据块: 单个文件内部数据组成的块, 大小与逻辑块相同, 从 1 开始编号,
数据块号仅在包含它的文件内部有效.

在外部表现上, 逻辑块与磁盘块唯一的不同在于在磁盘上的起始位置不同. 磁盘块从磁盘的第
1 块 512
字节块 就开始计数, 一直到磁盘的最后一块 512 字节块. 而逻辑块的第 1 块从磁盘的数据区
开始计数, 该磁盘块前面的几块要预留给超级块, i 结点位图, 逻辑块位图,
...
超级块含有整个文件系统的配置信息
	...
	struct m_super_block {
		...
		ino_t	inode_left;	/* 剩余 i 结点数 */
		blkcnt_t block_left;	/* 剩余 逻辑块数 */
	};
...
的磁盘. i 结点位图的每一个二进制位 (除了第 1 个位) 为 0,
...
逻辑块位图的第 1 个位不用, 在初始化文件系统时, 这两位被初始化为 1.
